...
public:
    ...
    int getMaxDepth()
    {
        return helperGetMaxDepth(this->root);
    }

private:
    int helperGetMaxDepth(Node *root)
    {

        if (root == nullptr)
            return 0;
        int leftDepth = helperGetMaxDepth(root->left);
        int rightDepth = helperGetMaxDepth(root->right);
...
int main()
{
    ...
    */
    ...
    parent == -1 ? cout << "Root" << endl : parent == -5 ? cout << "<<Not Found>>" << endl
